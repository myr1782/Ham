 ...
 * @version 1.0
 */
@Action(value = "department", results = { @Result(name = "success", location = "/page/sys/department.jsp"),
		@Result(name = "input", location = "/page/sys/department.jsp"),
		@Result(name = "edit", location = "/page/sys/departmentEdit.jsp"),
		@Result(name = "init", type = "redirect", location = "department!doInit.action"), })
public class DepartmentAction extends BaseAction implements ModelDriven<DepartmentPage> {
	/** 序列化ID */
	private static final long serialVersionUID = 175797690272212968L;
	/** 画面对应Model */
	private DepartmentPage page;
	/** Service，注解注入实例 */
	@Autowired
	private DepartmentServiceIF service;

	/**
	 * 取得画面对应Model的实例
	 */
	@Override
	public DepartmentPage getModel() {
		if (page == null) {
